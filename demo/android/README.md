...
3. In `app/build.gradle.kts`, replace `implementation(project(":ratex-android"))` with `implementation("io.github.erweixin:ratex-android:0.1.6")` and remove the `include(":ratex-android")` / `project(":ratex-android").projectDir` from `settings.gradle.kts`.
